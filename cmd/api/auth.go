...
func (app *application) getTokenFromHeaderAndVerify(w http.ResponseWriter, r *http.Request) (string, *Claims, error) {
	// expect authorization header to look like this:
	// Bearer <tokens>
	...
	// parse the token with our claims (read into claims), using our secret (from the receiver)
...
func (app *application) generateTokenPair(user *data.User) (TokenPairs, error) {
	...
	// set expiry; must be longer than jwt exp
