...
func Test_app_authenticate(t *testing.T) {
	var theTests = []struct {
		...
		{"not json", `I'm not a JSON`, http.StatusUnauthorized},
		{"empty json", `{}`, http.StatusUnauthorized},
		{"empty email", `{"email":""}`, http.StatusUnauthorized},
		{"empty password", `{"email":"admin@example.com"}`, http.StatusUnauthorized},
		{"empty password", `{"email":"admin@someotherdomain.com","password":"secret"}`, http.StatusUnauthorized},
	}
...
func Test_app_userHandlers(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			"insertUser invalid",
			"PUT",
			`{"foo":"bar","first_name":"Administrator","last_name":"User","email":"admin@example.com"}`,
			"",
			app.insertUser,
			http.StatusBadRequest,
		},
