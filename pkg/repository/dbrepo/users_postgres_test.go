...
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Fatalf("could not connect to docker; it is running? %s", err)
	}
	...
	// start the image and wait untill it's
	if err := pool.Retry(func() error {
		var err error
		testDB, err = sql.Open("pgx", fmt.Sprintf(dsn, host, port, user, password, dbName))
		if err != nil {
			log.Println("ErrorL", err)
			return err
		}
		return testDB.Ping()
	}); err != nil {
	...
	err = createTables()
	if err != nil {
		log.Fatalf("error createing tables: %s", err)
	}
...
func TestPostgresDBRepoAll(t *testing.T) {
...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error getting by id: %s", err)
	}
...
func TestPostgresDBRepoGetUserByEmail(t *testing.T) {
	...
	if user.ID != 2 {
		t.Errorf("wrong email returned by GetUserByEmail; expected jack@smith.com but got %d", user.ID)
	}
}
